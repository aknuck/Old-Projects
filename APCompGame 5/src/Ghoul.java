...
public class Ghoul extends Enemy{
	...
	// - Stone Dungeon
	// - Low stats
    public Ghoul(int l,int d, int x, int y, DunginGame gm) {
	 ...
	 public void move(Dungeon d, Player p){
		 Tile[][] map = d.getMap();
			...
			if (!((x-p.getX())*(x-p.getX())+(y-p.getY())*(y-p.getY()) < 25)){
				...
				if (dir.equals("x")){
					if (x > p.getX() && map[x-1][y].getType().equals("floor") && !map[x-1][y].hasEnemy()){
					...
					else if (x <= p.getX() && map[x+1][y].getType().equals("floor") && !map[x+1][y].hasEnemy()){
						if (map[x+1][y].hasPlayer()){
							//fight
						}
						else{
				...
				if (dir.equals("y")){
					if (y > p.getY() && map[x][y-1].getType().equals("floor") && !map[x][y-1].hasEnemy()){
					...
					else if (y <= p.getY() && map[x][y+1].getType().equals("floor") && !map[x][y+1].hasEnemy()){
						if (map[x][y+1].hasPlayer()){
							//fight
						}
						else{
