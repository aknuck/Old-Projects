...
public class WaterDungeon extends Dungeon{
	ArrayList<Room> rooms;
	//ArrayList<Item> possibleItems;
	Tile[][] map;

    public WaterDungeon(int dif, Player p, int lvl, DunginGame dm) {
    	super(dif,p,lvl,dm);
    	...
    	for (int i=0; i<numEnemies; i++){
    		while (true){
    			...
    			if (map[ranx][rany].type.equals("floor") && !map[ranx][rany].hasEnemy()){
    				...
    				if (pick==1){
    					enm = new Jellyfish(LEVEL,dif,ranx,rany,gm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==2){
    					enm = new Shark(LEVEL,dif,ranx,rany,gm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==3){
    					enm = new Grassgolem(LEVEL,dif,ranx,rany,gm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==4){
    					enm = new Icegolem(LEVEL,dif,ranx,rany,gm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				break;
	...
	private Tile[][] convertToTile(String[][] board){
		...
		for (int x=0; x<board.length;x++){
			for (int y=0; y<board[0].length;y++){
				if (board[x][y] != null){
					if (board[x][y].equals("@"))
						tileMap[x][y] = new Tile("solid",x,y,"waterDungeonSheet");
					else if (board[x][y].equals("+"))
						tileMap[x][y] = new Tile("wall",x,y,"waterDungeonSheet");
					else if (board[x][y].equals("."))
						tileMap[x][y] = new Tile("floor",x,y,"waterDungeonSheet");
					else if (board[x][y].equals("*"))
						tileMap[x][y] = new Tile("door",x,y,"waterDungeonSheet");
					else
						tileMap[x][y] = new Tile("solid",x,y,"waterDungeonSheet");
				}
				else
