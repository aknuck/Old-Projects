...
public class StoneDungeon extends Dungeon{
    ...
    public StoneDungeon(int dif, Player p, int lvl, DunginGame dm) {
    	super(dif,p,lvl,dm);
    	...
    	for (int i=0; i<numEnemies; i++){
    		while (true){
    			...
    			if (map[ranx][rany].type.equals("floor") && !map[ranx][rany].hasEnemy()){
    				...
    				if (pick==1){
    					enm = new Rat(LEVEL,dif,ranx,rany,dm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==2){
    					enm = new Skeleton(LEVEL,dif,ranx,rany,dm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==3){
    					enm = new Zombie(LEVEL,dif,ranx,rany,dm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				if (pick==4){
    					enm = new Ghoul(LEVEL,dif,ranx,rany,dm);
    					map[ranx][rany].setUnit(enm);
    					enemies.add(enm);
    				}
    				break;
	...
	private Tile[][] convertToTile(String[][] board){
		...
		for (int x=0; x<board.length;x++){
			for (int y=0; y<board[0].length;y++){
				if (board[x][y] != null){
					if (board[x][y].equals("@"))
						tileMap[x][y] = new Tile("solid",x,y,"stoneDungeonSheet");
					else if (board[x][y].equals("+"))
						tileMap[x][y] = new Tile("wall",x,y,"stoneDungeonSheet");
					else if (board[x][y].equals("."))
						tileMap[x][y] = new Tile("floor",x,y,"stoneDungeonSheet");
					else if (board[x][y].equals("*"))
						tileMap[x][y] = new Tile("door",x,y,"stoneDungeonSheet");
					else
						tileMap[x][y] = new Tile("solid",x,y,"stoneDungeonSheet");
				}
				else
